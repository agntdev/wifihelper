...
composer.callbackQuery(/^wifi:security:(WPA2|WPA3|WEP|Open)$/, async (ctx) => {
  await ctx.answerCallbackQuery(); if (!(await owner(ctx))) return; const security = ctx.match[1] as Security; const draft = ctx.session.draft; if (!draft?.ssid || draft.password === undefined) { await ctx.editMessageText("That setup timed out. Start Wi‑Fi settings again."); return; }
...
composer.on("message:text", async (ctx, next) => {
  ...
  if (ctx.session.flow === "wifi_password") { ctx.session.draft = { ...ctx.session.draft, password: text.toLowerCase() === "skip" ? "" : text }; await ctx.reply("Choose the network security type.", { reply_markup: inlineKeyboard([SECURITY.map((s) => inlineButton(s, `wifi:security:${s}`))]) }); return; }
  return next();
});
