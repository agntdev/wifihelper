...
export async function showGuests(ctx: Ctx, page = 0, edit = false): Promise<void> {
  ...
  const text = rows.map((t) => `${t.name || "Guest"} · expires ${t.expiresAt.replace("T", " ").slice(0, 16)} UTC · ${t.id}`).join("\n");
  const keys = rows.map((t) => [inlineButton(`Revoke ${t.name || "guest"}`, `revoke:${t.id}`), inlineButton("Details", `guest:${t.id}`)]);
  ...
  if (edit) await ctx.editMessageText(`Active guests (${active.length})\n${text}`, { reply_markup: inlineKeyboard(keys) }); else await ctx.reply(`Active guests (${active.length})\n${text}`, { reply_markup: inlineKeyboard(keys) });
}
